...
The **mobile nav** is also built: below the `sm` breakpoint the inline link list collapses into a hamburger toggle, with `aria-expanded`/`aria-controls` kept in sync, Escape closing the panel and returning focus to the button, and 44px touch targets. The panel uses the `hidden` attribute rather than a class, so it leaves the accessibility tree when closed instead of merely going invisible. The script is ~380 bytes and Astro inlines it, so it costs no extra request.
...
**Phase 5 is done.** The project galleries open into a lightbox. This started as PhotoSwipe, but its per-page rebuild kept fighting the view transitions added in Phase 8 (a first click that did nothing, an overlay opening below the fold after navigating back and returning), so it was replaced with a small dependency-free overlay — a fixed full-screen layer built in JS on click, delegated to `document` so it survives client-side navigation. The `images` schema entries are objects carrying `src`/`width`/`height`/optional `alt`; the dimensions reserve the thumbnail's space to prevent layout shift. The array requires at least one entry, since the portfolio grid uses `images[0]` as each card's thumbnail.
...
View transitions (`<ClientRouter />`) are the larger change, because they alter how every script on the site must be written — see CLAUDE.md. The mobile nav and the lightbox both delegate their listeners to `document`, which persists across the swap, so neither needs per-page re-initialisation. Both are covered by tests that navigate client-side first, since a page-two regression is invisible to a test that only loads pages directly.
...
**Phase 7 has started.** The first real content is in: a photo-only "Nick's Garden" project (seven optimised JPEGs served from `public/`), home-page copy grounded in those photos, and a full-bleed hero image on the home page. The hero is the first use of the `astro:assets` pipeline the plan calls for — `<Picture>` importing from `src/assets/`, emitting responsive AVIF (with a WebP fallback), eager-loaded and high-priority because it's the LCP element. A `bg-forest` scrim over it keeps the heading's WCAG AA contrast, which the a11y sweep enforces. The two placeholder projects and the placeholder About/business details remain.

Still to build: the rest of the real content — her About bio, real business name/contact details in `config.ts`, and more project photos (Phase 7); migrating the project galleries onto `<Image>`/`<Picture>` too; then the launch checklist (Phase 10), including a real OG image and flipping `SITE_INDEXABLE`.
